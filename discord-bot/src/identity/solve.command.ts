...
export class SolveChallengeCommand
  implements DiscordTransformedCommand<SolveDto>
{
  ...
  async handler(
    @Payload() dto: SolveDto,
    context: TransformedCommandExecutionContext,
  ) {
    // length check
    if (dto.challenge?.length !== 130) {
      this.reply(
        `Signed challenge must be exactly 130 symbols. You sent a string with ${dto.challenge.length} symbols`,
        context,
      );
      return;
    }
    ...
    if (verification) {
      this.logger.debug(
        `Verifying that challenge '${verification.challenge}' signature '${dto.challenge}' was signed by address '${verification.claimedAccountAddress}'`,
      );
      try {
        const { isValid } = signatureVerify(
          verification.challenge,
          dto.challenge,
          verification.claimedAccountAddress,
        );
        if (!isValid) {
        ...
        this.logger.debug(
          `Verification failed for '${verification.claimedAccountAddress}'`,
        );
        this.reply(`Signed challenge you provided is not correct`, context);
        return;
      }
      ...
      if (existingBinding) {
        this.logger.log(
          `Identity '${existingBinding.membership}' already claimed by '${existingBinding.discordHandle}'`,
        );
        ...
        if (created) {
          this.logger.log(
            `${this.buildHandle(context.interaction)} claimed identity '${
              verification.claimedMembership
            }'`,
          );
          ...
          const serverUser = await context.interaction.guild?.members.fetch({
            user: context.interaction.user,
          });

          await serverUser?.roles.add(
            verifiedRole.id,
            'Verified via claiming on-chain identity',
          );

          this.reply(
            `Congrats! You have successfully claimed the identity. Your on-chain roles should show up within 30 minutes`,
            context,
          );
          return;
        } else {
          ...
          this.reply(
            `Well, this is embarassing, but I have to ask you to try again later.`,
            context,
          );
          return;
        }
      ...
      this.logger.log(
        `No pending verification for user ${this.buildHandle(
          context.interaction,
        )}`,
      );
      this.reply(
        `You don't have any pending verifications. Claim your Joystream account using \`/claim\` command`,
        context,
      );
      return;
    }
