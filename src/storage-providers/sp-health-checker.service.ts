...
export class StorageProviderHealthChecker {
  ...
  async reportFaultyNodes() {
    ...
    badNodes.forEach(
      async (badNode: UnhealthyStorageProvider, index: number) => {
        const bags = await this.pioneerClient.getStorageBagsByNodeEndpoint(
          badNode.endpoint,
        );
        if (!this.allBagsHaveSingleBucket(bags)) {
          badNodesSummaryList = badNodesSummaryList.concat(
            `${index + 1}. ${badNode.endpoint}\n`,
          );
        }
      },
    );
    ...
    if (badNodesSummaryList !== '') {
      ...
      const storageProvidersWorkingGroupRole = await findServerRole(
        this.client,
        serverToCheck,
        wgToRoleMap[SP_CHANNEL_KEY],
      );
      const channelToUse = findDiscordChannel(
        this.client,
        channelNames[SP_CHANNEL_KEY],
      )[0];
      channelToUse.send({
        embeds: [
          getFaultyNodesEmbed(
            badNodesSummaryList,
            storageProvidersWorkingGroupRole,
          ),
        ],
      });
      ...
      endpoints.map(async (endpoint: string) => {
        try {
          const pingResponse = await axios.get(
            `${endpoint}api/v1/state/data`,
            axiosConfig,
          );
          if (pingResponse.status !== 200) {
            this.logger.warn(
              `Node ${endpoint} health check failed. HTTP status: ${pingResponse.status}`,
            );
            const bags = await this.pioneerClient.getStorageBagsByNodeEndpoint(
              endpoint,
            );
  ...
  allBagsHaveSingleBucket(bags: GetStorageBagsByNodeEndpointQuery): boolean {
    ...
    return (
      bags.storageBuckets.find((sb) =>
        sb.bags.find((bag) => bag.storageBuckets.length !== 1),
      ) === undefined
    );
  }

  async storeFaultyNode(endpoint: string) {
    const exists = await this.unhealthyStorageProviderRepository.count({
